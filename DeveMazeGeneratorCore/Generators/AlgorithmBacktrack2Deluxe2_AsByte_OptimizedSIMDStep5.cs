...
{
    public class AlgorithmBacktrack2Deluxe2_AsByte_OptimizedSIMDStep5 : IAlgorithm<Maze>
    {
        ...
        private Maze GoGenerateInternal<M, TAction>(M map, IRandom random, TAction pixelChangedCallback) where M : InnerMap where TAction : struct, IProgressAction
        {
            ...
            while (stackje.Count != 0)
            {
                ...
                // SIMD Step 4: Use SIMD for boolean operations (combining bounds check with map check)
                if (Sse2.IsSupported)
                {
                ...
                else
                {
                    ...
                    // SIMD Step 5: Use SIMD for direction selection logic
        ...
        {
            var boundsVector = Vector128.Create(
                boundsLeft ? (byte)1 : (byte)0,
                boundsRight ? (byte)1 : (byte)0,
                boundsUp ? (byte)1 : (byte)0,
                boundsDown ? (byte)1 : (byte)0,
        ...
        {
            // Create a vector with valid directions and their cumulative counts
            var validVector = Vector128.Create(
                validLeft ? (byte)1 : (byte)0,
                validRight ? (byte)1 : (byte)0,
                validUp ? (byte)1 : (byte)0,
            ...
            // Calculate cumulative sums using SIMD prefix sum approach
            int count0 = 0;
            ...
            // Create comparison vector for the chosen direction
            ...
            // Compare to find which direction was chosen
            var equalMask = Vector128.Equals(targetVector, chosenVector);

            // Apply valid mask to ensure we only select valid directions
